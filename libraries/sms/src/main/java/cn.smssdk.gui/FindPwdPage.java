...
/** 短信注册页面*/
public class FindPwdPage extends FakeActivity implements OnClickListener,
		TextWatcher {
...
//			}
			...
			etPhoneNum.setHint("填写注册或绑定的手机号码");
	...
	public void showDialog(final String phone, final String code) {
		...
		if (resId > 0) {
			...
			if (layout != null) {
				...
				((Button) dialog.findViewById(Res.id.btn_dialog_ok)).setText("取消");
				...
				((Button) dialog.findViewById(Res.id.btn_dialog_cancel)).setText("确定");
...
//									Log.e("verification phone ==>>", phone);
	...
	private void initActionBar(){
		...
		if(actionBar != null){
			...
			actionBar.setCustomView(R.layout.pub_actionbar_1);
			
			ViewGroup group = (ViewGroup)actionBar.getCustomView();
			
			ImageButton button = (ImageButton)group.getChildAt(0);
			...
			TextView title = (TextView)group.getChildAt(1);
			title.setText(getStringRes(activity, "smssdk_regist"));
