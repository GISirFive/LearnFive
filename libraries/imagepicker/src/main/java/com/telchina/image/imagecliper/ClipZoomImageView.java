...

{
	...
	public void onGlobalLayout() {
		if (once) {
			...
			// mScaleMatrix.postTranslate((width - dw) / 2, (height - dh) /
			// 2);// 平移至屏幕中心
			// mScaleMatrix.postScale(scale, scale, getWidth() / 2,
			// getHeight() / 2);// 设置缩放比例
			mScaleMatrix.postTranslate(ClipImageBorderView.mCircleCenterX
					- (dw / 2), ClipImageBorderView.mCircleCenterY - (dh / 2));// 平移至屏幕中心
			// 图片移动至屏幕中心
			setImageMatrix(mScaleMatrix);
			once = false;
		}
	...
	public Bitmap clip() {
		...
		return getCircleBitmap(Bitmap
				.createBitmap(
						bitmap,
						(int) (mScreenWidth * 0.191),
						(int) ((mScreenHeight - (ClipImageBorderView.mRadius) * 2) / 2),
						ClipImageBorderView.mRadius * 2,
						ClipImageBorderView.mRadius * 2));
	}
